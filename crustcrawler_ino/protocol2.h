...
// Extern - declaring buffers but with no memory allocation:
...
// Send a WRITE instruction to the servo:
void send_write_instruction(uint8_t id, uint16_t address, uint8_t data); // 1 byte
void send_write_instruction(uint8_t id, uint16_t address, uint16_t data); // 2 bytes
void send_write_instruction(uint8_t id, uint16_t address, int32_t data); // 4 bytes
